...
def test_command_arg_function_call_in_config():
    code = "w.config(command=foo())"
    assert lint(code) == {
        "1:18 TK111 Calling `foo()` instead of referencing it for `command`. Perhaps you meant `command=foo` (without the parentheses)?"
    }


def test_bind_function_call():
    code = "w.bind('<Button-1>', foo(), add=True)"
    assert lint(code) == {
        "1:22 TK111 Calling `foo()` instead of referencing it for bind. Perhaps you meant `foo` (without the parentheses)?"
    }
